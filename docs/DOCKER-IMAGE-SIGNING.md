...
1. Builds multi-architecture Docker images (amd64 and arm64) using Docker Buildx
2. Generates SBOM and provenance attestations automatically via Buildx
3. Pushes images and attestations to GitHub Container Registry (ghcr.io)
4. Signs the image digest with Cosign using GitHub's OIDC token
5. Records all signatures in the public transparency log
...
# View complete image manifest (includes all attestations)
docker buildx imagetools inspect ghcr.io/marre/pem2jks:latest
```
...
Attestations (SBOM and provenance) are automatically signed by GitHub Actions and can be verified using Cosign:

```bash
# Verify attestations (includes SBOM and provenance)
cosign verify-attestation ghcr.io/marre/pem2jks:latest \
  --certificate-identity-regexp="https://github.com/marre/pem2jks/.*" \
  --certificate-oidc-issuer="https://token.actions.githubusercontent.com"

# Verify and display the attestation payload
cosign verify-attestation ghcr.io/marre/pem2jks:latest \
  --certificate-identity-regexp="https://github.com/marre/pem2jks/.*" \
  --certificate-oidc-issuer="https://token.actions.githubusercontent.com" \
