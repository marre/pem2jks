...
These features ensure the authenticity, integrity, and transparency of the images. Both the SBOM and provenance attestations are **automatically cryptographically signed by GitHub Actions** using Sigstore's keyless signing infrastructure when pushed to the registry. The image digest itself is also signed separately using Cosign.
...
4. **Automatically signs the SBOM and provenance** using GitHub Actions' OIDC integration with Sigstore
5. Pushes images, SBOM, and signed attestations to GitHub Container Registry (ghcr.io)
6. Uses Cosign to additionally sign the image digest with GitHub's OIDC token
7. Records all signatures in the public transparency log
...
**Note on Attestation Signing:** The SBOM and provenance attestations generated by Docker Buildx are automatically signed using GitHub Actions' OIDC integration with Sigstore. This is separate from the explicit Cosign signing of the image digest in the workflow. Both the image and its attestations are cryptographically verifiable using Cosign.
