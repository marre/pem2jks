...
- **Can append to existing PKCS#12 keystores**
...
# Add CA certificates to existing truststore
...
| `--input` | `-i` | Existing keystore file to append to (PKCS#12 only) |
...
- **Appending to JKS**: Currently, appending to existing JKS keystores is not supported. You must recreate the entire JKS from scratch with all desired certificates. Appending only works with PKCS#12 format.
