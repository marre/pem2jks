...
pem2jks supports **FIPS 140-2 compliant** keystore generation. When using PKCS#12 format (with `--format=pkcs12` or `--fips` flag), the tool generates keystores using FIPS-approved cryptographic algorithms:
...
Enable FIPS mode to ensure only FIPS-compliant formats are used:

```bash
# Using --fips flag (automatically uses PKCS#12 format)
...
# Explicit PKCS#12 format for FIPS compliance
pem2jks -c tls.crt:tls.key -p changeit --format=pkcs12
```

**Important Notes:**
- **JKS format is NOT FIPS-compliant** as it uses SHA-1 for integrity checks and key encryption. When JKS is used in non-FIPS mode, a warning will be displayed.
- **PKCS#12 format is FIPS-compliant** when using the Modern encoder (default in pem2jks).
- In FIPS mode, attempting to use JKS format will result in an error.
...
| `--fips` | | Enable FIPS 140-2 compliant mode (only PKCS#12 format allowed) |
| `--help` | `-h` | Show help |
...
| `FIPS_MODE` | Enable FIPS 140-2 compliant mode (set to `true` or `1`) |

### Output Formats

| Format | Description | Use Case | FIPS 140-2 Compliant |
|--------|-------------|----------|----------------------|
| `jks` | Java KeyStore (uses SHA-1) | Traditional Java applications | ❌ No |
  ...
  initContainers:
    - name: cert-converter
      ...
      args:
        ...
        - --fips  # Enable FIPS 140-2 compliant mode
      env:
        - name: FIPS_MODE
          value: "true"  # Alternative: use environment variable
      volumeMounts:
        - name: certs
          mountPath: /certs
          readOnly: true
