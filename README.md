...
A pure Rust utility to convert PEM certificates (like those generated by Kubernetes cert-manager) into Java KeyStore (JKS) format.

## Features

- Converts PEM certificates and private keys to JKS format
- Supports RSA and EC private keys
- Handles certificate chains
- **Supports multiple certificate/key pairs in a single keystore**
- **Supports multiple CA certificates from different files**
- **Can append to existing JKS keystores**
...
pem2jks --input truststore.jks --ca new-ca.crt:newca \
        -p changeit
```
...
| `--input` | `-i` | Existing JKS keystore file to append to |
| `--input-password` | | Password for input keystore (defaults to `--password` if not specified) |
| `--output` | `-o` | Output keystore file path (default: keystore.jks) |
...
The `Dockerfile` is a minimal scratch-based image that copies the pre-built binary. Release images are built, pushed, and signed automatically using cross-compilation and Docker Buildx.
